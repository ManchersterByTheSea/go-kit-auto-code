...
const (
	...
	templateD = "package service\n\nimport (\n\t\"context\"\n\n\t\"xtech-kit/const\"\n\t\"xtech-kit/error\"\n\t\"xtech-kit/proto/{{service}}/v1\"\n\t{{service}}cligrpc \"xtech-kit/svc/{{service}}-svc/client/grpc\"\n\n\t\"github.com/xianyu-tech/tlog\"\n\t\"github.com/xianyu-tech/util/log\"\n)\n\nfunc (p *basic{{Service}}ApiService) {{Name}}{{method}}(ctx context.Context, reqproto *{{service}}proto.{{Name}}{{method}}ReqProto) (*{{service}}proto.{{Name}}{{method}}RespProto, error) {\n\tdefer func() {\n\t\tif rec := recover(); rec != nil {\n\t\t\terrMsg := tlog.Error(\"login logs get (%d, %d, %d, %d) err (recover %v).\",\n\t\t\treqproto.GetStaffId(), reqproto.GetQueryStaffId(), reqproto.GetFrom(), reqproto.GetLimit(), logutil.PrintPanic())\n\n\t\t\ttlog.AsyncSend(tlog.NewRawLogError(comconst.ErrPanicRecovery(\"login logs get\"), errMsg))\n\t\t}\n\t}()\n\t\n\t{{name}}{{method}}RespProto := &{{service}}proto.{{Name}}{{method}}RespProto{\n\t\tErrCode: errconst.COMMON_API_ERROR_OK,\n\t}\n\n\treturn {{name}}{{method}}RespProto, nil\n}\n"
)
